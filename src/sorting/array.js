...
// special array operations to handle sparse arrays in a sensible fashion.
    function SparseSlice(array, start_i, del_count, len, deleted_elements) {
        ...
        var indices =
    %
        GetArrayKeys(array, start_i + del_count);
        if (IS_NUMBER(indices)) {
            var limit = indices;
...
// length of the array. See ECMA-262, section 15.4.4.7.
    function ArrayPush() {
        ...
        if (m > kMaxSafeInteger - n) throw
    %
        make_type_error(kPushPastSafeLength, m, n);
...
// For implementing reverse() on large, sparse arrays.
    ...
    function ArrayShift() {
        ...
        throw
    %
        make_type_error(kArrayFunctionsOnSealed);
    ...
    function ArraySplice(start, delete_count) {
        ...
        %
            ...
            throw
        %
            make_type_error(kArrayFunctionsOnFrozen);
        }
    ...
    function InnerArraySort(array, length, comparefn) {
        ...
        function CopyFromPrototype(obj, length) {
            ...
            proto =
        %
            object_get_prototype_of(proto)
        )
            {
                var indices = IS_PROXY(proto) ? length :
            %
                GetArrayKeys(proto, length);
                if (IS_NUMBER(indices)) {
                    // It's an interval.
        ...
        %
            ...
            {
                var indices = IS_PROXY(proto) ? to :
            %
                GetArrayKeys(proto, to);
                if (IS_NUMBER(indices)) {
                    // It's an interval.
                    var proto_length = indices;
...
// or delete elements from the array.
    ...
    function ArrayFilter(f, receiver) {
        ...
        var array = TO_OBJECT(this);
        var length = TO_LENGTH(array.length);
        if (!IS_CALLABLE(f)) throw
    %
        make_type_error(kCalledNonCallable, f);
...
// (Array.prototype.copyWithin ( target, start [ , end ] )
    ...
    function InnerArrayFind(predicate, thisArg, array, length) {
        if (!IS_CALLABLE(predicate)) {
            throw
        %
            make_type_error(kCalledNonCallable, predicate);
        }

        for (var i = 0; i < length; i++) {
            var element = array[i];
            if (%
            _Call(predicate, thisArg, element, i, array)
        )
            {
                return element;
            }
    ...
    function ArrayFrom(arrayLike, mapfn, receiver) {
        ...
        if (mapping) {
            if (!IS_CALLABLE(mapfn)) {
                throw
            %
                make_type_error(kCalledNonCallable, mapfn);
            }
        }
        ...
        %
            ...
            for (nextValue of
                ...
                if (mapping) {
                    mappedValue =
                %
                    _Call(mapfn, receiver, nextValue, k);
                } else {
            ...
            result =
        %
            IsConstructor(this) ? new this(len) : new GlobalArray(len);
...
// ES6, draft 05-22-14, section 22.1.2.3
    function ArrayOf(...args) {
        ...
        var array =
    %
        IsConstructor(constructor) ? new constructor(length) : [];
        for (var i = 0; i < length; i++) {
        %
